...
//                + "your browser.");
        ...
        if(lrlat > ullat || lrlon < ullon || ullat < ROOT_LRLAT || ullon > ROOT_LRLON
                || lrlat > ROOT_ULLAT || lrlon < ROOT_ULLON) {
            results.put("query_success" , false);
        }

        double queryLonDPP = lonDPP(lrlon, ullon, w);
        double fileLonDPP = lonDPP(ROOT_LRLON, ROOT_ULLON, TILE_SIZE);
        int depth = 0;

        while(queryLonDPP < fileLonDPP && depth < 7){
            depth ++ ;
        ...
        double rasterUllat = rasterHelperLAT(ROOT_ULLAT, ullat, dLat, ulat);
    ...
    private double lonDPP(double lrlon, double ullon, double w){
        double numerator = lrlon - ullon;
        double answer = numerator / w;
        return answer;
    }

    private double distancePerTile(double x, double y, int d){
        double numerator = x - y;
        double denominator = (int) Math.pow(2, d);
